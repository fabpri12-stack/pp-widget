...
type EventSeriesRow = {
  ...
  standard_ticket_label?: string | null;
  standard_ticket_price_display?: string | null;
  standard_ticket_price?: string | number | null;
  standard_ticket_description?: string | null;
  primary_ticket_label?: string | null;
  primary_ticket_price_display?: string | null;
  primary_ticket_price?: string | number | null;
  primary_ticket_description?: string | null;
  secondary_ticket_enabled?: boolean | string | number | null;
  secondary_ticket_label?: string | null;
  secondary_ticket_price_display?: string | null;
  secondary_ticket_price?: string | number | null;
  secondary_ticket_description?: string | null;
  second_ticket_enabled?: boolean | string | number | null;
  second_ticket_label?: string | null;
  second_ticket_price_display?: string | null;
  second_ticket_price?: string | number | null;
  second_ticket_description?: string | null;
...
type ScheduleRuleDate =
  ...
  | {
      ...
      available_times?: ScheduleRuleTime[] | string | null;
    };
...
type ScheduleRuleRow = {
  ...
  selected_dates?: ScheduleRuleDate[] | string | null;
...
function toBoolean(value: unknown) {
  ...
  if (typeof value === "string") return ["true", "1", "yes", "enabled", "on"].includes(value.trim().toLowerCase());
  return false;
}
...
function firstTicketOption(value: unknown) {
  const options = toArray<any>(value);
  return options.find((item) => item && typeof item === "object" && toBoolean(item.enabled ?? item.active ?? true)) ?? null;
}

function standardTicketDetails(event: EventSeriesRow) {
  const label = cleanText(event.standard_ticket_label ?? event.primary_ticket_label) ?? "Standard";
  const priceDisplay = cleanText(
    event.standard_ticket_price_display ??
    event.primary_ticket_price_display ??
    event.standard_ticket_price ??
    event.primary_ticket_price ??
    event.price_display ??
    event.default_price_display,
  );
  const description = cleanText(event.standard_ticket_description ?? event.primary_ticket_description) ?? "Standard show night ticket.";

  return {
    standardTicketLabel: label,
    standardTicketPriceDisplay: priceDisplay,
    standardTicketDescription: description,
  };
}

function secondaryTicketDetails(event: EventSeriesRow) {
  const option = firstTicketOption(event.ticket_options);
  const label = cleanText(
    event.secondary_ticket_label ??
    event.second_ticket_label ??
    option?.label ??
    option?.name ??
    option?.title,
  );
  const priceDisplay = cleanText(
    event.secondary_ticket_price_display ??
    event.second_ticket_price_display ??
    event.secondary_ticket_price ??
    event.second_ticket_price ??
    option?.price_display ??
    option?.price ??
    option?.amount,
  );
  const description = cleanText(
    event.secondary_ticket_description ??
    event.second_ticket_description ??
    option?.description ??
    option?.text,
  );
  const enabled = toBoolean(event.secondary_ticket_enabled ?? event.second_ticket_enabled ?? option?.enabled ?? option?.active) || Boolean(label || priceDisplay || description);

  return {
    secondaryTicketEnabled: enabled,
    secondaryTicketLabel: label,
    secondaryTicketPriceDisplay: priceDisplay,
    secondaryTicketDescription: description,
  };
}
...
function normaliseScheduleDate(value: ScheduleRuleDate) {
  if (typeof value === "string") {
    const date = value.trim();
    return date ? { date, status: null, times: null } : null;
  }

  if (!value || typeof value !== "object") return null;

  const date = cleanText(value.date ?? value.event_date ?? value.clean_date);
  if (!date) return null;

  return {
    date,
    status: cleanText(value.status ?? value.availability_status ?? value.date_status),
    times: value.times ?? value.available_times ?? null,
  };
}

function mergeDateAndRuleTimes(dateValue: ScheduleRuleDate, ruleTimes: Array<{ time: string; status?: string | null }>, ruleStatus?: string | null) {
  const date = normaliseScheduleDate(dateValue);
  if (!date) return null;

  const dateTimes = normaliseScheduleTimes(date.times);
  const baseTimes = dateTimes.length ? dateTimes : ruleTimes;
  const times = baseTimes.length ? baseTimes : [{ time: "19:00", status: null }];
  const dateStatus = mapStatus(date.status ?? ruleStatus) === "sold_out" ? "sold_out" : date.status;

  return {
    date: date.date,
    status: dateStatus ?? ruleStatus ?? null,
    times: times.map((slot) => ({
      ...slot,
      status: dateStatus === "sold_out" ? "sold_out" : slot.status ?? dateStatus ?? ruleStatus ?? null,
    })),
  };
}
...
function createSession(input: {
  ...
  return {
    ...
    doors: input.doors ?? "At least 30 minutes before show time",
    status,
...
function sessionsFromScheduleRules(
  ...
  const rangeStart = new Date(today.getFullYear(), today.getMonth(), today.getDate(), 12);
  const rangeEnd = addDays(rangeStart, 420);
  const todayIso = isoDate(rangeStart);
  ...
  for (const event of events) {
    ...
    for (const rule of eventRules) {
      if (rule.active !== undefined && rule.active !== null && !toBoolean(rule.active)) continue;
      ...
      const selectedDates = toArray<ScheduleRuleDate>(rule.selected_dates);

      if (selectedDates.length) {
        selectedDates.forEach((dateValue) => {
          const dateInstance = mergeDateAndRuleTimes(dateValue, sessionTimes, rule.status);
          if (!dateInstance || excludedDates.has(dateInstance.date)) return;
          if (dateInstance.date < todayIso) return;
          dateInstance.times.forEach((slot, index) => {
            byEvent[event.id].push(createSession({ event, bookingType, date: dateInstance.date, time: slot.time, duration, doors: rule.doors, status: slot.status ?? dateInstance.status ?? rule.status, suffix: `dates-${index}`, scheduleRuleId: rule.id }));
          });
        });
...
function mapEvents(
  ...
  return events
    .map((event) => {
      ...
      const standardTicket = standardTicketDetails(event);
      const secondaryTicket = secondaryTicketDetails(event);
      ...
      return {
        ...
        priceFromPence: parseMoneyToPence(standardTicket.standardTicketPriceDisplay ?? event.price_display ?? event.default_price_display, 2995),

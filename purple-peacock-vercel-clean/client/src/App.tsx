...
function getRequestedEventFilters() {
...
function EventsCalendar() {
  ...
  const [urlEventFilterIds, setUrlEventFilterIds] = useState<string[] | null>(null);
    ...
    return events
      ...
      .map((event) => ({
        ...
        matchValues: [
          ...
          normaliseFilterValue(event.slug),
  ...
  useEffect(() => {
    ...
    const requestedFilters = getRequestedEventFilters();
    if (!requestedFilters.length) {
      setUrlEventFilterIds(null);
      return;
    }

    const matchedIds = eventOptions
      .filter((option) => requestedFilters.some((request) => option.matchValues.includes(request)))
      .map((option) => option.id);

    const uniqueMatchedIds = Array.from(new Set(matchedIds));
    setUrlEventFilterIds(uniqueMatchedIds.length ? uniqueMatchedIds : []);
    if (uniqueMatchedIds.length === 1) setEventFilter(uniqueMatchedIds[0]);
    if (uniqueMatchedIds.length > 1) setEventFilter("all");
  }, [eventOptions]);

  const filteredSessions = useMemo(() => {
    if (urlEventFilterIds) return sessions.filter((session) => urlEventFilterIds.includes(session.event.id));
    if (eventFilter === "all") return sessions;
    return sessions.filter((session) => session.event.id === eventFilter);
  }, [eventFilter, sessions, urlEventFilterIds]);

...
const dmnBookingConfig = {
  ...
  source: "NovaAI",
  returnUrl: "https://www.purplepeacockncl.co.uk/thank_you_nova",
};
...
function buildDmnHandoffUrl(session: EventSession, customer: typeof defaultCustomer) {
  const params = new URLSearchParams({
    ...
    return_url: dmnBookingConfig.returnUrl,
  });
...
function AvailabilityStep({
  ...
  const hasChecked = availabilityState === "available" || availabilityState === "unavailable" || availabilityState === "error";
  const isChecking = availabilityState === "checking";
  const hasGuests = customer.guests > 0;
  const canContinue = availabilityState === "available";
  ...
  async function checkAvailability() {
    ...
    try {
      ...
      setAvailabilityResult({
        ...
        valid: false,
        action: "error",
        message: error instanceof Error ? error.message : "Availability check failed. Please try again.",
      });
  ...
  return (
    <div className="booking-step">
      ...
      {hasChecked ? (
        <div className={`availability-result ${canContinue ? "" : "pending"}`} data-testid="status-availability">
          {canContinue ? <ShieldCheck size={20} /> : <CircleAlert size={20} />}
          <div>
            <strong>{availabilityResult?.message ?? "Availability checked."}</strong>
            <p>
              Selected: {event.title}, {formatLongDate(session.date)}, {session.time}. Deposit due at payment:
              {" "}{formatMoney(event.depositPence * customer.guests)}.
            </p>
      ...
      <div className="button-pair">
        ...
        {!hasChecked ? (
          ...
          <Button data-testid="button-continue-details" className="book-button" disabled={!canContinue} onClick={onContinue}>Continue to details</Button>
        )}
      </div>
    </div>
  );
...
function TransferStep({
  ...
  return (
    <div className="booking-step transfer-screen" data-testid="screen-dmn-transfer">
      ...
      <h3>Transferring you to the secure payment system via DesignMyNight</h3>
      <p data-testid="status-transfer">
        {customer.firstName || "Guest"}, your booking details are prepared. Continue to DesignMyNight to complete
        the secure payment or pre-order step.
      </p>
      ...
      <div className="button-pair">
        <Button data-testid="button-back-details" variant="outline" className="glass-action" onClick={onBack}>Back</Button>
        <Button
...
function ConfirmationStep({
  ...
  return (
    <div className="booking-step confirmation-screen" data-testid="section-confirmation">
      <div className="confirmed-icon"><Check size={24} /></div>
      <p className="eyebrow">Returned from DesignMyNight</p>
      <h3>Booking received, {customer.firstName || "guest"}</h3>
      <div className="booking-receipt">
        <p><strong>Reference</strong><span>{bookingRef}</span></p>
        ...
        <p><strong>Deposit paid</strong><span>{formatMoney(depositTotal)}</span></p>
      </div>
      ...
      <div className="button-pair">
        ...
        <Button className="book-button" data-testid="button-download-welcome">

  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="glass-modal border-white/15 bg-[#14091f]/88 text-white backdrop-blur-2xl sm:max-w-3xl">
        ...
        {mode === "menu" && (
          <div className="modal-grid menu-choice-grid" data-testid="modal-menu">
            {menuLinks.length ? menuLinks.map((link) => (
              <a key={`${link.label}-${link.url}`} className="menu-card menu-link-card" href={link.url} target="_blank" rel="noreferrer">
                {menuIcon(link.label)}
                <strong>{link.label}</strong>
                <p>{link.description || "Open menu in a new tab."}</p>
              </a>
            )) : (

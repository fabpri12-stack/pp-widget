...
function EventsCalendar() {
  ...
  useEffect(() => {
    if (!sessions.length || pendingPaymentSnapshot) return;
    try {
      ...
      const pendingSession = findReturnSession(sessions, pending);
      if (!pendingSession) return;
      setPendingPaymentSnapshot(pending);
      setSelectedDate(pendingSession.date);
      setSelectedSessionId(pendingSession.id);
      setSelectedEventId(pendingSession.event.id);
      setPaymentFlowState("awaiting");
      setBookingStep("transfer");
    } catch {
      // Ignore pending payment restore failures.
    }
  }, [pendingPaymentSnapshot, sessions]);

...
function getSelectedTicketOption(event: CalendarEvent, choice: TicketOptionChoice) {
  ...
  return {
    ...
    label: "Standard",
    price: formatMoney(event.priceFromPence),
    description: "Standard show night ticket",
  };
}
...
function buildDmnHandoffUrl(session: EventSession, event: CalendarEvent, customer: typeof defaultCustomer) {
  ...
  returnUrl.searchParams.set("guests", String(customer.guests));
  returnUrl.searchParams.set("first_name", customer.firstName);
  ...
  const params = new URLSearchParams({
    ...
    internal_notes: buildDmnNotes(event, customer),
...
function EventsCalendar() {
  ...
  }, [eventFilter, filteredSessions, selectedSessionId]);
  ...
  useEffect(() => {
    ...
    if (returnMessage.guests > 0 || returnMessage.firstName) {
      setCustomer((current) => ({
        ...current,
        guests: returnMessage.guests > 0 ? returnMessage.guests : current.guests,
        firstName: returnMessage.firstName || current.firstName,
      }));
    }
  ...
  useEffect(() => {
    function applyReturnMessage(rawValue: string | null) {
      ...
      try {
        ...
        if (message.guests > 0 || message.firstName) {
          setCustomer((current) => ({
            ...current,
            guests: message.guests > 0 ? message.guests : current.guests,
            firstName: message.firstName || current.firstName,
          }));
        }
  ...
  useEffect(() => {
    ...
    try {
      ...
      const pending = JSON.parse(rawPending) as ReturnType<typeof createPaymentReturnMessage> & { createdAt?: number };
      if (pending.createdAt && Date.now() - pending.createdAt > PAYMENT_PENDING_TTL_MS) return;

      if (pending.sessionId) setSelectedSessionId(pending.sessionId);
      if (pending.eventId) setSelectedEventId(pending.eventId);
      if (pending.date) setSelectedDate(pending.date);
      if (pending.guests > 0 || pending.firstName) {
        setCustomer((current) => ({
          ...current,
          guests: pending.guests > 0 ? pending.guests : current.guests,
          firstName: pending.firstName || current.firstName,
        }));
      }
      setBookingReturn(pending.bookingReturn);
      setAvailabilityState("available");
      setPaymentFlowState("idle");
      setBookingStep("confirmation");
        ...
        <div className={`ticket-options-grid ${event.secondaryTicketEnabled ? "has-premium" : ""}`} data-testid={`section-ticket-options-${event.id}`}>
          ...
          >
            <div className="ticket-option-main">
              <span><Ticket size={16} /> Standard</span>
              <strong>{formatMoney(event.priceFromPence)}</strong>
            </div>
            <p>Standard show night ticket.</p>
          </button>
  ...
  return (
    <div className="booking-step transfer-screen" data-testid="screen-dmn-transfer">
      ...
      <p data-testid="status-transfer">
        {paymentPopupBlocked
          ? "Your browser blocked the payment window. Use the secure payment button below to open DesignMyNight in a new tab."
          : paymentFlowState === "awaiting"
      ...
      <div className="payment-summary">
        ...
        <span>{customer.guests} guests · {getSelectedTicketOption(event, customer.ticketOption).label} ticket · Deposit {formatMoney(depositTotal)}</span>
      </div>
      <div className="button-pair">
